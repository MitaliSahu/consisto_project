...
function handleAuth() {
    ...
    if(user.trim() !== "") showPage('page-dashboard');
...
function addGoal() {
    ...
    if(!title.trim()) { alert("Please name your milestone!"); return; }

    goals.push({ id: Date.now(), title: title.trim(), freq: freq, checks: {}, created: new Date().toLocaleDateString() });
        ...
        div.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                <div onclick="viewGoal(${goal.id})" style="cursor: pointer; flex-grow: 1;">
                    <strong style="font-size: 1.1rem;">${goal.title}</strong>
                    <small style="opacity: 0.6; display: block;">Target: ${goal.freq}</small>
                </div>
                <button class="delete-task-btn" onclick="deleteGoal(${goal.id}, event)">Delete</button>
            </div>
            <div onclick="viewGoal(${goal.id})" style="cursor: pointer;">
                <div class="progress-bar"><div class="progress-fill" style="width:${progress}%"></div></div>
                <div style="text-align: right; margin-top: 5px;">
                    <span style="font-size: 12px; color: var(--primary); font-weight: bold;">${progress}%</span>
                </div>
            </div>
        `;
...
window.onload = () => { if(goals.length > 0) renderDashboard(); };